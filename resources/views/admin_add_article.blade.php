...
<form class="p-5 col-lg-6 mx-auto" method="POST" action="/admin/articles/add" enctype="multipart/form-data">
    ...
    <textarea class="form-control mb-2 @error('description') is-invalid @enderror" id="description" value="" name="description" placeholder="Карточное описание">{{ old('description') }}</textarea>
    @error('description')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
    ...
    <textarea class="form-control mb-2 @error('content') is-invalid @enderror" id="content" value="" name="content" placeholder="Полное описание">{{ old('content') }}</textarea>
    @error('content')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
